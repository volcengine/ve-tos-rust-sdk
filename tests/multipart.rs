...
use ve_tos_rust_sdk::multipart::{AbortMultipartUploadInput, CompleteMultipartUploadInput, CreateMultipartUploadInput, ListMultipartUploadsInput, ListPartsInput, MultipartAPI, UploadPartCopyInput, UploadPartFromBufferInput, UploadPartFromFileInput, UploadPartInput, UploadedPart};
...
fn test_multipart_upload(context: &Context) {
    ...
    let key = key.as_str();
    let o = client.create_multipart_upload(&CreateMultipartUploadInput::new(bucket, key)).unwrap();
    ...
    if meta.is_err() {
        {
            let mut fd = File::options().write(true).truncate(true).create(true).open(big_file_path).unwrap();
    ...
    let o = client.list_parts(&ListPartsInput::new(bucket, key, upload_id)).unwrap();
    assert!(o.request_id().len() > 0);
    assert_eq!(o.parts().len(), 2);

    let o = client.complete_multipart_upload(
        &CompleteMultipartUploadInput::new_with_parts(bucket, key, upload_id, Vec::from([UploadedPart::new(1, etags[0].as_str()), UploadedPart::new(2, etags[1].as_str())]))).unwrap();
    assert!(o.request_id().len() > 0);
    ...
    let o = client.create_multipart_upload(&CreateMultipartUploadInput::new(bucket, key2)).unwrap();
    assert!(o.request_id().len() > 0);
    assert!(o.upload_id().len() > 0);
    ...
    let o = client.complete_multipart_upload(
        &CompleteMultipartUploadInput::new_with_parts(bucket, key2, upload_id2, Vec::from([UploadedPart::new(1, etags[0].as_str()), UploadedPart::new(2, etags[1].as_str())]))).unwrap();
    assert!(o.request_id().len() > 0);
    assert!(o.hash_crc64ecma() > 0);
    println!("complete parts done, {}", o.hash_crc64ecma());
    ...
    let o = client.create_multipart_upload(&CreateMultipartUploadInput::new(bucket, key3)).unwrap();
    assert!(o.request_id().len() > 0);
    assert!(o.upload_id().len() > 0);
    ...
    let o = client.complete_multipart_upload(
        &CompleteMultipartUploadInput::new_with_complete_all(bucket, key3, upload_id3, true)).unwrap();
    assert!(o.request_id().len() > 0);
    assert!(o.hash_crc64ecma() > 0);
    println!("complete parts done, {}", o.hash_crc64ecma());
    ...
    let mut o = client.get_object(&GetObjectInput::new(bucket, key3)).unwrap();
    assert!(o.request_id().len() > 0);
    let target_md5 = base64_md5(read_to_buf(o.content().unwrap()));
    assert_eq!(source_md5, target_md5);
}
...
fn test_multipart_list(context: &Context) {
    ...
    for (idx, key) in keys_array.iter().enumerate() {
        ...
        let o = client.create_multipart_upload(&CreateMultipartUploadInput::new(bucket1, keys[idx].as_str())).unwrap();
        assert!(o.request_id().len() > 0);
        assert!(o.upload_id().len() > 0);
    ...
    for i in 0..50 {
        let o = client.abort_multipart_upload(&AbortMultipartUploadInput::new(bucket1, keys[i].as_str(), upload_ids.get(keys[i].as_str()).unwrap())).unwrap();
        assert!(o.request_id().len() > 0);
    }
    ...
    for i in 1..3 {
        ...
        keys.push(key.clone());
        let o = client.create_multipart_upload(&CreateMultipartUploadInput::new(bucket1, key)).unwrap();
        assert!(o.request_id().len() > 0);
    }
    ...
    let e = client.create_multipart_upload(&CreateMultipartUploadInput::new(context.non_exists_bucket(), gen_random_string(3))).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchBucket");

    let e = client.list_multipart_uploads(&ListMultipartUploadsInput::new(context.non_exists_bucket())).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchBucket");

    let e = client.upload_part_from_buffer(&UploadPartFromBufferInput::new(context.non_exists_bucket(), gen_random_string(3), "123")).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchBucket");

    let e = client.abort_multipart_upload(&AbortMultipartUploadInput::new(context.non_exists_bucket(), gen_random_string(3), "123")).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchBucket");
    ...
    let e = client.upload_part_from_buffer(&UploadPartFromBufferInput::new(bucket, gen_random_string(400), upload_id)).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchUpload");

    let e = client.upload_part_from_buffer(&UploadPartFromBufferInput::new(bucket, key, "123")).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchUpload");

    let e = client.list_parts(&ListPartsInput::new(bucket, gen_random_string(400), upload_id)).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchUpload");

    let e = client.list_parts(&ListPartsInput::new(bucket, key, "123")).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchUpload");

    let e = client.abort_multipart_upload(&AbortMultipartUploadInput::new(bucket, gen_random_string(400), upload_id)).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchUpload");

    let e = client.abort_multipart_upload(&AbortMultipartUploadInput::new(bucket, key, "123")).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchUpload");

    let e = client.complete_multipart_upload(&CompleteMultipartUploadInput::new_with_parts(bucket, gen_random_string(400), upload_id, Vec::from([UploadedPart::new(1, "123")]))).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchUpload");

    let e = client.complete_multipart_upload(&CompleteMultipartUploadInput::new_with_parts(bucket, key, "123", Vec::from([UploadedPart::new(1, "123")]))).expect_err("");
    assert!(e.is_server_error());
    let ex = e.as_server_error().unwrap();
    assert_eq!(ex.status_code(), 404);
    assert_eq!(ex.code(), "NoSuchUpload");
